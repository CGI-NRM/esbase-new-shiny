...
params:
    ...
    provlistas_homogenat: NA
    provlistas_analyslab: NA
header-includes:
  - \usepackage{booktabs}
...
for (n in names(params$provlistas)) {
    cols <- c("provid")
    if (params$provlistas_analyslab[[n]] == "ACES") {
        cols <- c(cols, "aces")
    }
    if (params$provlistas_homogenat[[n]]) {
        cols <- c(cols, "delvikt")
    }
...
# TODO: column_spec with min-width for provid
knitr::kable(
