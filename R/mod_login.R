...
font-size: 18px; font-weight: 600;"),
    ...
    shiny::observeEvent(input$jscookie, {
      ...
      if (!is.null(input$jscookie) && uuid::UUIDvalidate(input$jscookie) && file.exists(paste0("active_sessions/", input$jscookie))) {
        account$id <- readLines(paste0("active_sessions/", input$jscookie))
